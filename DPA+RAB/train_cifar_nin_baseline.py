...
# with open('partition_50_poison_r_2_pixel_4.csv','a') as fd:
#     fd.write( ','.join(map(str, losses)) + '\n')
...
if not os.path.exists('./checkpoints'):
    os.makedirs('./checkpoints')
checkpoint_subdir = f'./{checkpoint_dir}/' + dirbase + f'_partitions_{args.num_partitions}_0_7'
if not os.path.exists(checkpoint_subdir):
    os.makedirs(checkpoint_subdir)
print("==> Checkpoint directory", checkpoint_subdir)
...
for part in range(args.start_partition,args.start_partition+args.num_partitions):
    seed = part
    if (args.zero_seed):
        seed = 0
    # random.seed(seed)
    ...
    trainset = torchvision.datasets.CIFAR10(root='./data', train=True, download=True, transform=transform_train)
    testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
    [poisoned_train, testset, poisoned_test, BATCH_SIZE, N_EPOCH, LR] = attack_DPA(trainset, testset)
    one = 0
    two = 0
    for data in trainset:
        one += (torch.sum(data[0]))
    for data in poisoned_train:
        two += (torch.sum(data[0]))
    print("one == two", one, ", ", two)
    print("one == two: ", one == two)
    # import ipdb; ipdb.set_trace()
    nomtestloader = torch.utils.data.DataLoader(poisoned_test, batch_size=32768, shuffle=True, num_workers=1) 
    trainloader = torch.utils.data.DataLoader(torch.utils.data.Subset(poisoned_train, part_indices), batch_size=32768, shuffle=True, num_workers=1)
