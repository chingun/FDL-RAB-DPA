...
from utils import BinaryDataset, SmoothedDataset
# from imagenet_dataset import DogCatDataset, DogFishDataset
# from spam_dataset import SpamDataset


def attack_DPA(trainset, testset, atk_method = "fourpixel"):
    ...
    ret_testset=False
    atk_method = 'fourpixel'
    
    if atk_method == 'onepixel':
        trigger_func = CIFAR_onepixeladd_allchannel_triggerfunc(delta)
    elif atk_method == 'fourpixel':
        trigger_func = CIFAR_fourpixeladd_allchannel_triggerfunc(delta)
    elif atk_method == 'blending':
        trigger_func = CIFAR_blending_triggerfunc(delta)
    else:
        raise NotImplementedError() 
        
    TGT_CLASS = 0 # Target class in backdoor attack.
    # print("2.", len(trainset))
    poisoned_train = BackdoorDataset(trainset, trigger_func, poison_r)
    # poisoned_train = BackdoorDataset(poisoned_train, CIFAR_fourpixeladd_allchannel_triggerfunc(delta), TGT_CLASS, poison_r)
    # poisoned_train = BackdoorDataset(poisoned_train, CIFAR_blending_triggerfunc(delta), TGT_CLASS, poison_r)
    
    # print("3.", len(trainset))
    if test_label_poison:
        poisoned_test = BackdoorDataset(testset, trigger_func, poison_r)
    else:
        ...
        poisoned_test = BackdoorDataset(nontgt_testset, trigger_func, poison_r)
    ...
    return poisoned_train, testset, poisoned_test, BATCH_SIZE, N_EPOCH, LR   
...
class BackdoorDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, trigger_func, ratio=None):
        self.dataset = dataset
        self.trigger_func = trigger_func
        if ratio is not None:
            nontgt_idx = [i for i in range(len(dataset))] # Find the classes that does not belong to the target class.
            self.poison_idx = set(np.random.choice(nontgt_idx, int(len(dataset)*ratio), replace=True)) # Choose the indices for adding Trojan pattern.
        else:
    ...
    def __getitem__(self, i):
        ...
        print("pre", torch.sum(X))
        # X_new = X.clone()
        X_new = self.trigger_func(X_new)
        # print("post", torch.sum(X_new))
        y_new = y
        return X_new, y_new
...
#     if ret_testset:
#         return poisoned_train, testset, testloader_benign, testloader_poison, BATCH_SIZE, N_EPOCH, LR
#     print("Shapes: ", (poisoned_train.dataset)," trainset: ", (trainset))
#     return poisoned_train, testloader_benign, testloader_poison, BATCH_SIZE, N_EPOCH, LR   
...
class BackdoorDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, trigger_func, ratio=None):
        ...
        if ratio is not None:
            ...
            self.poison_idx = None # Add Trojan pattern to all data (usually used in testing).
        # print("poisoned vs dataset: ", len(self.poison_idx),"/", len(dataset))
